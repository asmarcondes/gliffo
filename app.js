      ...
      function nextCursor(from) {
        ...
        const last = from - 1;
        if (last >= 0 && !G.decoded.has(last) && !G.keyPos.has(last))
          return last;
      ...
      function handleKey(k) {
        ...
        if (k === "⌫") {
          ...
          if (
            ...
            let prev = c - 1;
            while (prev >= 0 && (G.decoded.has(prev) || G.keyPos.has(prev)))
              prev--;
            if (prev >= 0) {
              G.cursor = prev;
              if (G.typed[prev]) {
      ...
      function calcWarns() {
        ...
        if (!HARD_MODE) {
          ...
          for (let i = 0; i < WN; i++) {
            ...
            if (
              G.attempts.some((a) => a.word[i] === l && !a.decoded.includes(i))
            )
              posErrada.push({ l, i });
          }
          ...
          if (G.decoded.size === WN) {
            ...
            (function () {
              ...
              if (spHour >= 2 && spHour < 4) {
                ...
                setTimeout(() => {
                  setFb(
                    "Ainda acordado às " + spHour + "h? 🌙 Vai dormir!",
                    "ok",
                  );
      ...
      function _winReveal(overlay, yc) {
        ...
        yc.addEventListener(
          "animationend",
          () => yc.classList.remove("win-reveal"),
          { once: true },
        );
        setTimeout(() => {
      ...
      function _doWinFinale(stage, stack, layers, overlay, yc, ISO_OUT) {
        ...
        if (f === "A") {
          ...
          stage.style.transition =
            "transform 550ms ease-in, opacity 450ms ease-in";
      ...
      function atualizarStats(won, tentativas) {
        ...
        checkAchievements({
          ...
          stats: s,
          shared: false,
          guess: G.attempts.length
            ? G.attempts[G.attempts.length - 1].word
            : null,
    ...
    <div class="stat-card"><div class="stat-val" style="font-size:1.2rem;letter-spacing:0.1em">${WORD}</div><div class="stat-lbl">Palavra</div></div>`;
        ...
        if (winDef)
          winDef.href = `https://www.dicio.com.br/${WORD.toLowerCase()}/`;
        openM("win-modal");
      }
      ...
      function loseMod() {
        ...
        if (loseDef)
          loseDef.href = `https://www.dicio.com.br/${WORD.toLowerCase()}/`;
      ...
      async function sharePassport() {
        ...
        try {
          const s = carregarStats();
          const pct =
            s.jogados > 0 ? Math.round((s.vitorias / s.jogados) * 100) : 0;
          ...
          const maxVal = Math.max(
            1,
            ...chaves.map((k) => s.distribuicao[k] || 0),
          );
      ...
      function syncConfigUI() {
        ...
        if (soundSub)
          soundSub.textContent = AUDIO_MUTED ? "Desativado" : "Ativado";
      ...
      function toggleHardMode() {
        if (G.attempts.length > 0 && !G.done) {
          setFb(
            "Não é possível mudar o Modo Difícil durante uma partida.",
            "warn",
          );
          return;
        }
      ...
      document.addEventListener("keydown", (e) => {
        ...
        if (e.key === "Backspace") handleKey("⌫");
          ...
          while (c >= 0 && c < WN && (G.decoded.has(c) || G.keyPos.has(c)))
            c += dir;
        ...
        if (rankEl) {
          if (titulo.proximo) {
            ...
            const pctBar = Math.round(
              ((s.streakAtual - prev) / (cur - prev)) * 100,
            );
  ...
  <div class="rank-info"><div class="rank-name">${titulo.nome}</div></div>
      ...
      <div class="distrib-bar-wrap">
        ...
        const statsShareSection = document.getElementById(
          "stats-share-section",
        );
      ...
      function showAchPopup(ach) {
        document.getElementById("ach-popup-icon").textContent =
          ach.emoji || "🏅";
      ...
      function checkAchievements({
        ...
        if (G.attempts.some((a) => achIsAnagram(a.word, WORD)))
          queueAch("echo");
        ...
        if (G.attempts.some((a) => achIsPalindrome(a.word)))
          queueAch("palindrome");
    ...
    <span class="ach-ribbon-text" style="color:${p.ribbon}">${name}</span>
      ...
      function buildHeaderMeta(refDate) {
        ...
        const difficultyLabel =
          CURRENT_PUZZLE?.difficultyLabel ||
          CICLO_NAMES[difficulty] ||
          "Puzzle";
    ...
    <span class="hard-badge">🔥 Difícil</span>`
          ...
          if (idx < buildOrder.length) {
            ...
            isoLoop = setTimeout(() => {
              ...
              layers.forEach((lay) => {
                ...
                lay.style.transform =
                  "translateZ(0px) rotateX(0deg) rotateZ(0deg)";
              });
      ...
      function startTyping(word, slotsId, yoursId, interval, onDone, token) {
        ...
        function next() {
          ...
          if (i < word.length) {
            ...
            const disp = [
              ...typed,
              ...new Array(TWL.length - typed.length).fill(""),
            ];
      ...
      function installTutKeyHandler() {
        ...
        tutKeyHandler = (e) => {
          ...
          if (k === "BACKSPACE") {
            ...
            const expected = iLetters.map((l, i) =>
              iConfirmed.has(i) ? null : l,
            );
      ...
      function renderStep(step, noClear) {
        ...
        switch (step) {
          ...
          case 1: {
            ...
            chatMsg(
              "Para entender melhor... Vamos juntar as letras!",
              allIn1 + 400,
            );
                    ...
                    const runPhases = () => {
                      const doRun = () => {
                        ...
                        const targetTX = tileWrappers.map(
                          (el) =>
                            rowCenterX - (el.offsetLeft + el.offsetWidth / 2),
                        );
                      ...
                      if (activeSplitRow && activeSplitRow.parentNode) {
                        anime({
                          ...
                          complete: () => {
                            if (activeSplitRow && activeSplitRow.parentNode)
                              activeSplitRow.parentNode.removeChild(
                                activeSplitRow,
                              );
                            activeSplitRow = null;
                            doRun();
          ...
          case 2: {
            ...
            tDelay(() => {
              if (!token.cancelled && stack2)
                tStackEl(stack2, TDL, () => tStroke());
            }, 600);
            ...
            function runGuess2(word) {
              ...
              if (num2 === 1) {
                tDelay(() => {
                  ...
                  const _bbClick = () => {
                    ...
                    eggBubble.textContent =
                      "Ei! O menino só passa, não morde! 🍌";
                    if (feed && typingEl) {
                  ...
                  const doFrame = (ts) => {
                    ...
                    bbDiv.style.right =
                      (-BB_W + eio(Math.min(t / T_WALK, 1)) * DIST).toFixed(0) +
                      "px";
              ...
              chatCard((card) => {
                ...
                const { panel: dp2, stack: ds2 } =
                  makeGlyphPanel("Glifo do Dia");
                gSt2 = ds2;
              ...
              tDelay(() => {
                ...
                if (!isP2) {
                  ...
                  setTimeout(() => {
                    pulseEls.forEach((el) =>
                      el.classList.add("tut-found-pulse"),
                    );
                  }, 200);
                  setTimeout(() => {
                    pulseEls.forEach((el) =>
                      el.classList.remove("tut-found-pulse"),
                    );
              ...
              if (isP2) {
                ...
                const startCaro = (cameFromBirra) => {
                  ...
                  tried2.forEach((w) =>
                    wFound2[w].forEach((l) => coloredSet.add(l)),
                  );
                            ...
                            const launchInteractive = (delay) => {
                              ...
                              tDelay(() => {
                                ...
                                if (tutKeyRevealedPos !== null) {
                                  iTyped[tutKeyRevealedPos] =
                                    TDL[tutKeyRevealedPos];
                                ...
                                const feed =
                                  document.getElementById("tut-chat-feed");
                                if (!feed) return;
                      ...
                      chatSetChoices([
                        ...
                        },
                        {
                          ...
                          onClick: () => {
                            ...
                            if (introBubble)
                              introBubble.innerHTML = introHtmlMasked;
                            chatMsg(
                ...
                const birraRound2 = () => {
                  chatMsg(
                    `Hmm. Não sei não... você me parece muito suspeito. 🤨`,
                    0,
                  );
                  tDelay(() => {
                    chatSetChoices([
      ...
      function _installHijack(onSend) {
        ...
        const phrase =
          BYE_PHRASES[Math.floor(Math.random() * BYE_PHRASES.length)];
        let phraseIdx = 0;
        input.value = "";
      ...
      function closeTutorial() {
        ...
        const _tutPanel = document.querySelector(
          "#tutorial-overlay .tut-panel",
        );
      ...
      window.addEventListener("load", () => {
        ...
        if ("serviceWorker" in navigator) {
          navigator.serviceWorker
            ...
            .catch(() => {}); // silencia erros em dev/file://
        }
      });
      document.querySelector('.hbtn[title="Ajuda"]').onclick = () =>
        openTutorial();
      ...
      function buildArquivoList() {
        ...
        const todayActual = new Date(
          todayBase.getTime() + dbgOffset * 86400000,
        );
        ...
        const numWeeks =
          Math.round((curSunday - epochDay) / (7 * 86400000)) + 1;
        ...
        for (let w = numWeeks - 1; w >= 0; w--) {
          ...
          const showMonthLabel =
            w === numWeeks - 1 ||
            weekSunday.getMonth() !== weekAbove.getMonth();
          if (showMonthLabel) {
          ...
          for (let d = 0; d < 7; d++) {
            ...
            const daysFromRealToday = Math.round(
              (dayDate - todayBase) / 86400000,
            );
            ...
            if (isToday) {
              ...
              if (raw) {
                try {
                  ...
                  usedKey = !!(
                    ds.keyUsed ||
                    (ds.keyPos && ds.keyPos.length > 0)
                  );
                  if (ds.done) {
                    status = ds.won ? "won" : "lost";
                    attempts =
                      ds.won && ds.attempts ? ds.attempts.length : null;
      ...
      (function initDebugMode() {
        ...
        function ativarDebug() {
          ...
          console.log(
            "[glif] Modo debug ativado. Ctrl+Shift+D para desativar.",
          );
        ...
        window.addEventListener("load", () => {
          ...
          ["current", "A", "B", "C", "D", "E"].forEach((v) => {
            safe(`dbg-fin-${v}`, () => {
              ...
              document
                .getElementById(`dbg-fin-${v}`)
                .classList.add("dbg-fin-active");
            });
          });
          safe("dbg-fin-test", () => {
          ...
          safe("dbg-ee-toast", () =>
            showEEToast("🧨 Toast de debug ativado!", 3000),
          );
        });
      })();
        ...
        document.addEventListener("keydown", (e) => {
          if (e.key === SEQ[p]) {
            ...
            if (p === SEQ.length) {
              ...
              setTimeout(
                () => document.body.classList.remove("ee-konami"),
                3000,
              );
            }
          } else {
      ...
      (function () {
        ...
        function _setChannel(idx, play) {
          ...
          if (_eeChSwitch >= 5) {
            _eeChSwitch = 0;
            setTimeout(
              () => showEEToast("😅 Indeciso? Cada canal é bom!"),
              300,
            );
          }
        }
