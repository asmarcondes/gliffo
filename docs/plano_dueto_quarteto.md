...
O jogo base do Gliffo já é _mais difícil_ que o Wordle em tentativas/board (4 vs 6).
...
O teclado serve apenas para orientação _positiva_ (o que foi encontrado/confirmado).
...
se for verdade em _qualquer_ board. A posição exata vem do Glifo/slots de cada board.
...
→ { words: ["ROSA","BALA"], difficulty: "facil", difficultyLabel: "Fácil", puzzle: 42, date: "2026-03-12" }

GET /functions/v1/daily-word?mode=quarteto
→ { words: ["ROSA","BALA","MESA","CANA"], difficulty: "facil", puzzle: 42, date: "2026-03-12" }
```
...
- [ ] Resposta: `{ words: string[], difficulty, difficultyLabel, puzzle, date }` (sempre array)
...
_leitura e síntese multi-board_, não da escassez de tentativas.
